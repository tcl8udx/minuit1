...
double calcChiSq(TH1F* h, TF1* f){
  double chisq=0;
  for (int i=1; i<=h->GetNbinsX(); i++){
    double x=h->GetBinCenter(i);
    double n=h->GetBinContent(i);
    double sigma=h->GetBinError(i);
    if (sigma<1e-10) continue;  // skip bins with no error
    double mu=f->Eval(x);
    chisq += TMath::Power((n-mu)/sigma, 2);
  }
  return chisq;
}
